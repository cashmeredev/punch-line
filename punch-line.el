...
(define-minor-mode punch-line-mode
  ...
  (if punch-line-mode
      (progn
        ...
        (punch-line-update)
        (when (and (fboundp 'punch-line-get-music-service)
                   (punch-line-get-music-service))
          (punch-line-start-music-info-timer))
        (when (and (bound-and-true-p punch-show-weather-info)
                   (fboundp 'punch-weather-enable-focus-hooks))
          (punch-weather-enable-focus-hooks))
        (when (and (bound-and-true-p punch-show-macro)
                   (fboundp 'punch-macro-register-hooks))
          (punch-macro-register-hooks)))
    (setq-default mode-line-format (default-value 'mode-line-format))
    (punch-line-remove-hooks)
    (when (fboundp 'punch-line-stop-music-info-timer)
      (punch-line-stop-music-info-timer))
    (when (fboundp 'punch-weather-disable-focus-hooks)
      (punch-weather-disable-focus-hooks))
    (when (fboundp 'punch-macro-unregister-hooks)
      (punch-macro-unregister-hooks))
    (force-mode-line-update t)))
